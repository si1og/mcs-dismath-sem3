...
\subsection{Константные значания}

Сперва стоит привести константные значания, так как на их основе строится логика вывода и валидации.

\begin{lstlisting}[language=C++, caption=Константные значания]
...
\begin{itemize}
  \item Так, \verb|RECOMMENDED_MAX_DEPTH| отвечает за значения размерности униврсума, до которых программа работает быстро (т.е. время выполнения любой операции $ \leq 1000$ мс) и стабильно, подробнее об этом в пункте <<Результаты работы программы>>.
  \item \verb|MAX_DEPTH|~--- жёсткое ограничение размерности, связанное с количеством битов в \verb|int|.
  \item \verb|PRINT_IN_TABLE_VIEW|~--- отвечает за количество отображаемых первых и последних строк в потраничной таблице.
  \item \verb|TABLE_MODE_DEPTH_TOGGLE|~--- отвечает за пепеключение режима отображения таблицы, когда элементы из-за размерности начинают занимать слишком много места в ячейках. 
\end{itemize}
...
Метод \texttt{symmetricDifferenceWith} вычисляет симметрическую разность как объединение двух разностей: $(A \setminus B) \cup (B \setminus A)$.
